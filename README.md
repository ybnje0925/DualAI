...
4. DualAI 사이트 탭으로 돌아와 확장 아이콘 → **DualAI Bridge** → **이 웹사이트 연결**을 누릅니다.
...
답변은 iframe 안에 복제하지 않고 실제 서비스 창에서 봅니다. 대시보드 탭과 두 서비스 창을 상하·좌우 배치합니다.
창 자동 배치가 환경에서 제한되면 직접 옮길 수 있습니다.
모바일 브라우저는 지원 대상이 아니며 데스크톱 Chrome/Edge를 사용하세요.

Enter/Shift+Enter는 줄바꿈, Ctrl+Enter(Windows)/Cmd+Enter(macOS)는 전송입니다.
실패한 서비스만 재시도할 수 있습니다. 전송 여부가 불확실하면 실제 화면을 확인한 뒤 새 대화를 시작하세요.
확장 변경 사항을 받으려면 새 ZIP을 같은 폴더에 풀고 확장 관리 화면에서 새로고침하세요.
